    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="details clearfix">
            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="changeOrder('1')">
                  <a href="#">综合 <span v-if="isOne" class="iconfont" :class="{'icon-up':isAsc,'icon-down':isDesc}"></span></a>
                </li>
                <li :class="{active:isTwo}" @click="changeOrder('2')">
                  <a href="#">价格 <span v-if="isTwo" class="iconfont" :class="{'icon-up':isAsc,'icon-down':isDesc}"></span></a>
                </li>
              </ul>
            </div>
          </div>
          ...
          <!-- 分页器 -->
          <!-- pageNo当前页 -->
          <Pagniation :pageNo="searchParams.pageNo" :pageSize="searchParams.pageSize" :total="total" :continues="4" @getPageNo="getPageNo"></Pagniation>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  beforeMount() {
    // 点击搜索时将数据传给searchParams然后将具体数据传递给服务器
  ...
  // }
  computed: {
    // mapGetters简化数据格式为数组形式 因此直接以数组形式获取数据 不是对象形式
  ...
  methods: {
    ...
    removeCategory() {
      ...
      //  再次发请求并且制空id
      // 带给服务器的参数说明都是可有可无的，弱国属性值为空的字符串，还是会把相应的字段带给服务器
      // 但是你把相应的undefined，当前这个字段不会传递给服务器
    ...
    // watch数据监听：监听组件上的属性发生变化
    watch: {
      // 监听路由的信息是否发生变化，如果发生变化再次发送请求
      $route() {
        // 每一次请求完毕之后应该把相应的一二三级id清空，让后续操作获取一级二级三级id
